"""Scale PTIE self-play from ppo_adp.pth. Stop only at WP>=0.90 vs random."""
...
ADP_GRAD_CLIP = 40.0
MAX_UPDATES = 80
DEFAULT_MIN_GAMES = 64
EVAL_DEALS = 40
EVAL_SLICE_START = 80
SAVE_BEST = "ppo_adp_scale.pth"
SAVE_CURVE = "reward_history_scale.png"
...
def main() -> None:
    ...
    for update in range(1, args.max_updates + 1):
        ...
        if update == 1 or update % EVAL_EVERY == 0 or update == args.max_updates:
            ...
            if is_strong_vs_random(wp, adp):
                print("reached strong ruler: WP>=0.90 and ADP>0 vs random")
                break
    ...
    if wp_history:
        ...
        fig.suptitle("Scaled PTIE self-play vs random")
        fig.tight_layout()
