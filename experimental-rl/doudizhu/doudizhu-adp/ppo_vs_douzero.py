"""PPO vs frozen DouZero-ADP. Critic target is whole-episode G. Best on that ruler."""
...
from vs_douzero import (
    DEFAULT_DZ_DIR,
    collect_vs_douzero,
    load_douzero_players,
    ppo_update_seat_mc,
)
...
SAVE_BEST = "ppo_adp_vs_dz.pth"
SAVE_CURVE = "reward_history_vs_dz.png"
...
def main() -> None:
    ...
    for update in range(1, args.max_updates + 1):
        batch = collect_vs_douzero(env, models, dz, args.min_games, ours="mix")
        losses = [
            ppo_update_seat_mc(
    ...
    if wp_history:
        ...
        ax1.set_xlabel("eval")
        ax1.set_ylabel("seat-swap WP vs DouZero-ADP")
        ax2 = ax1.twinx()
        ax2.plot(adp_history, color="tab:orange", label="ADP")
        ax2.set_ylabel("seat-swap ADP vs DouZero-ADP")
        fig.suptitle("PPO vs frozen DouZero-ADP (MC return)")
        fig.tight_layout()
