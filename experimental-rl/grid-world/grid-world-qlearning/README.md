...
```text
episode= 200  eps=0.937  train_return=...  greedy_mean=0.930
...
...
- `greedy_mean=0.930`：纯贪心已稳定走最短路。
- 策略图：从 `S` 出发跟箭头走，应绕过两堵竖墙与下方障碍，到达 `G`。
...
2. 打印整图贪心策略  
3. 连续跑 5 局纯贪心，打印坐标路径与 ASCII 地图  

用来确认：不是「平均分碰巧高」，而是每局都走出同一条（或等价的）短路径。
...
**Q: 训练回报偶尔很低，但 greedy_mean 已经 0.93？**  
A: 正常。训练仍在按 ε 随机探索；评估是纯贪心。
