...
**M++DP** 是问题本身（格子怎么转、怎么给分）；**TD** 是学这个问题的更新规则（每走一步怎么改 Q 表）。本例状态只有 25 个格子，Q 就是一张++ `(25, 4)` ++的表，每一步改哪个格子、改多少，都能对上代码。++

++**本例没有深度学习。** 学的是这张可查表的++ `Q`++，不是神经网++络；依赖只有 `numpy`，不需要 `torch`。深度网络在同级目录 `grid-world-dqn`。
