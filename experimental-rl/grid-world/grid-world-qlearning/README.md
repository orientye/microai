...
「马尔可夫」很具体：下一步只取决于**当前格子和当前动作**，不取决于是怎么走到这里的。环境 `step()` 走一步，只给三样东西：**(下一状态 `s'`，即时奖励 `r`，是否结束)**。这就是 MDP 的一次转移。Agent 要学的不是「这一步拿了多少分」，而是策略 `π(s)`：每个格子该往哪走，才能让**整条轨迹的折扣回报**尽量大。
...
**π(s) = argmax_a Q(s, a)**：站在格子 `s`，在四个方向里选 **Q 值最大的那个** 走。

| 符号 | 含义 |
| ---- | ---- |
| `s` | 当前格子（本例 `0..24`） |
| `a` | 动作 `0`↑ `1`→ `2`↓ `3`← |
| `Q(s, a)` | 在 `s` 选 `a`，从这一步起往后大概能攒多少折扣回报 |
| `argmax_a` | 在四个 `a` 里找 Q 最大的那个 |
| `π(s)` | 策略：格子 `s` 该往哪走 |

训练**直接学 Q 表**；策略 **π 是从 Q 表读出来的**，不是单独再学一张表。

**例子：起点 `(0,0)`，id=`0`**

```text
Q[0] = [ Q(↑), Q(→), Q(↓), Q(←) ]
     ≈ [ 0.85,  0.88,  0.82,  0.80 ]   # 训练后的示意值，真实数以 q_table.npy 为准
```

四个数里 **→ 最大** → `π(起点) = →`。代码即 `int(np.argmax(q[state]))`。

对每个**非障碍、非终点**格子都做一遍 `argmax`，就得到整图箭头（`policy_map.png` / `print_policy` 输出）：
...
- **箭头**：该格四个方向里 Q 最高的动作
- **跟箭头走**：从任意可行起点出发，应绕开 `#` 到达 `G`
- **颜色**（热力图）：`V(s) = max_a Q(s,a)`，即该格「最好那条路」的长期价值

**为什么叫「贪心」**：只看 Q 表里**当前最好**的动作，不再随机探索。训练时用 ε-greedy 偶尔乱试；评估 / 测试时 `ε=0`，全程 `argmax`（见 §6）。
...
上神经网络（DQN）之后，同样是这套 Bellman / TD 目标，但 `Q` 变成网络权重，看不到「格子 23 向右那一格被改了」。本例是查表：
...
要分两层：**一局**什么时候停，和**整个训练**什么时候停。

#### 单局（一个 episode）

每走一步，环境 `step()` 会判断本局是否结束：


| 条件 | 结果 |
|------|------|
| 踩到终点 `G` | `terminated=True`，本局结束 |
| 走了 **100 步**仍未到达 | `truncated=True`，本局结束（`grid_world_env.py` 里 `max_steps=100`） |


训练循环里 `done = terminated or truncated`，任一成立就结束当前局，然后 `reset` 开下一局。

#### 整个训练

**固定跑满 4000 局**，没有「学好了就提前停」的逻辑。


| 超参 | 值 | 含义 |
|------|-----|------|
| `MAX_EPISODES` | `4000` | 训练总局数，跑完就保存 Q 表并退出 |
| `EVAL_EVERY` | `200` | 每隔 200 局做一次全起点贪心评估，**只打印日志，不触发停止** |


代码里没有这些机制：

- 成功率到 100% 就停
- `all_starts_mean` 到 `0.93` 就停
- Q 表变化小于某个阈值就停
...
- 前 **3000 局**：ε 从 `1.0` 线性降到 `0.05`（`EPS_DECAY_EPISODES=3000`）
- 第 **3000～4000 局**：ε 固定在 `0.05`，训练仍继续

#### 实际表现

日志里往往 **200～600 局**左右就会出现 `success=100%`、`all_starts_mean≈0.93`，说明策略已基本学好；后面是在固定步数下继续微调 Q 表。若要「达标即停」，需自行加 early stopping（例如连续 N 次评估 `success_rate==1.0` 且 `mean_return` 达标）。
