...
`max` 里叠了两步：先在 `target_net` 上挑最大的动作，再用**同一个** `target_net` 给这个动作打分。Q 估计含噪声时，`max` 在统计上偏正：`E[max_a Q(s,a)] ≥ max_a E[Q(s,a)]`，噪声越大偏得越多。同一个网络既选又估，等于把噪声里的「运气好」当成「真的值这么多」，于是 `y` 系统性偏高，TD 把这个偏高反向传回 Q，过估计就滚起来。
