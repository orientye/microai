"""Soft Actor-Critic on Pendulum-v1 (twin Q + auto temperature)."""
...
BATCH_SIZE = 256
MEMORY_SIZE = 100_000
START_STEPS = 1_000  # uniform random actions before using policy
UPDATE_AFTER = 1_000
UPDATES_PER_STEP = 1
MAX_STEPS = 40_000
LOG_EVERY = 2_000
...
class SACAgent:
    def __init__(self, state_dim: int, action_dim: int, action_high: float):
        ...
        self.log_alpha = torch.zeros((), requires_grad=True)
...
def main() -> None:
    ...
    ax.set_title("Pendulum-v1 Soft Actor-Critic")
    ax.legend()
