...
```bash
cd experimental-rl/cliff-walking
...
grid-world-qlearning   固定小图，只看 Q-learning
       │
       ▼
cliff-walking（本目录）  同一环境对比 Q vs SARSA（离策略 / 在策略）
       │
       ▼
...
下一阶段可在**同一环境**上加神经网络对照，建议边界：

| 项 | 建议 |
|----|------|
| 目录 | `cliff-walking-dqn/`（并列）或本目录子文件夹 `dqn/` |
| 环境 | 仍 `CliffWalking-v1, is_slippery=False`；观察可用 one-hot / 坐标嵌入，不必上 CNN |
| 算法 | Double DQN（对齐 `cart-pole-dqn`）即可；主对照仍是「值函数近似 vs 表」 |
| 非目标 | 第一版不做 SARSA(λ) 深度版、不做 PPO；`is_slippery=True` 可作为可选消融 |
| 验收 | 贪心成功率高；策略是否仍贴崖可作为现象记录，不强制复现教科书图 |
