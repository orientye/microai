"""Soft Actor-Critic on LunarLanderContinuous-v3 (twin Q + auto temperature)."""
...
SAVE_BEST = "sac_lunar_lander.pth"
...
class ReplayBuffer:
    ...
    def sample(self, batch_size: int):
        ...
        return (
            ...
            torch.as_tensor(np.array(action), dtype=torch.float32, device=DEVICE),
...
class SACAgent:
    def __init__(self, state_dim: int, action_dim: int, action_high: float):
        self.action_high = float(action_high)
        self.actor = GaussianActor(state_dim, action_dim, action_high).to(DEVICE)
        self.q1 = QNet(state_dim, action_dim).to(DEVICE)
        self.q2 = QNet(state_dim, action_dim).to(DEVICE)
        self.q1_targ = QNet(state_dim, action_dim).to(DEVICE)
        self.q2_targ = QNet(state_dim, action_dim).to(DEVICE)
        ...
        # Auto temperature: maximize entropy toward -|A|
...
def main() -> None:
    ...
    print(f"Training SAC on LunarLanderContinuous-v3 (device={DEVICE})...")
    for step in range(1, MAX_STEPS + 1):
        ...
        memory.push(state, action, float(reward), next_state, float(terminated))
    ...
    ax.set_title("LunarLanderContinuous-v3 Soft Actor-Critic")
    ax.legend()
