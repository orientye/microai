...
对照说明：link:https://karpathy.github.io/2026/02/12/microgpt/[explainer]；gist：link:https://gist.github.com/karpathy/8627fe009c40f57531cb18360106ce95[microgpt.py]。 +
micrograd 拆解：xref:micrograd-Analysis.adoc[`micrograd-Analysis.adoc`]。 +
本文件按*这份克隆的真实源码*拆：link:microgpt/microgpt.py[`microgpt/microgpt.py`]（约 200 行，无依赖）。
...
| 文件
| `engine.py` + `nn.py` + demo
| *单文件* `microgpt.py`
| `Value` 图语义
| `_backward` 闭包
| `_local_grads` 元组（前向算死）
| 邻接
| `_prev = set(...)`
| `_children` *元组*（可重复）
| 可视化
...
| 原语
| `+ * ** relu`
| 再加 `log` / `exp`（为 softmax / CE）
| 网络
| `Neuron`/`Layer`/`MLP` 对象
| 无类：`linear` / `softmax` / `rmsnorm` / `gpt`
| 损失
| hinge + L2
| 逐步 `-log p(target)` 再平均
| 优化
| 手写 SGD：`p.data -= lr * p.grad`
| Adam + 线性衰减；更新后 `p.grad = 0`
| 任务
| moons 二分类
| 人名 next-token → 采样新名字
| 参数量
...
micrograd README 里说的进阶版就是这一句：_storing local gradients at forward time instead of per-op backward closures_。源码对得上。
...
和 xref:micrograd-Analysis.adoc[`micrograd-Analysis.adoc`] 的菱形是同一类图：`a` 被用两次，靠 `+=` 把路径加起来。
...
experimental-karpathy/
  ...
  microgpt-Analysis.adoc      # 本文件
  nanogpt-Analysis.adoc       # 相对本文：同一套 GPT，换成 PyTorch
...
microgpt 的 `Value` 更接近「把闭包求值提前」；要张量仍然得换 `microai` / PyTorch（对照 xref:nanogpt-Analysis.adoc[`nanogpt-Analysis.adoc`]）。
...
首次运行若没有 `input.txt`，下载 link:https://github.com/karpathy/makemore[makemore/names.txt]。约 32033 个名字，一行一个文档。
...
`rmsnorm`：`scale = (mean(x²) + 1e-5) ** -0.5`，再逐元乘。没有可学的 gain。嵌入后先做一次 rmsnorm；注释写 _not redundant due to backward pass via the residual connection_——残差旁路还握着未归一的 `x`，这条路上的梯度不经过后面的 norm。
...
| 数据结构
| `_prev` set + `_backward` 闭包
| `_children` 元组 + `_local_grads` 数表
| 前向
| 算 `data`，闭包捕获对象
| 算 `data`，*同时写出局部导* `(b,a)`、`(1,1)`
| 反向
| `reversed(topo)` 调闭包
| 同一拓扑，`zip` 做 `g += local * v.grad`
| 链式法则
| `∂e/∂a = 15+6=21`（菱形）
| `∂L/∂a = b+1=4`；`+=` 仍然是多路径求和
|===
...
| `_local_grads` + 统一 `backward`（比闭包干净）
| 在这份标量 `Value` 上加层数 / 加 vocab 当「真 GPT」
| 显式 KV cache（训练也穿过图）
| 把 cache 写成 detach 再当训练实现
| 逐步 CE + Adam 的最小循环
| 从 `microgpt.py` 抄并行、checkpoint、tokenizer
| 和 micrograd 对同一菱形 / `a+a`
| 闭包风格和新的数表混在一个 `Value` 里
|===
...
3. 再读 `gpt` + 训练循环。真要训语言模型，换 PyTorch（对照 xref:nanogpt-Analysis.adoc[`nanogpt-Analysis.adoc`]）或本仓库 `microai`，不要扩这个文件。
