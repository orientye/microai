...
- 先扫 §1～§3，搞清「它在算什么、文件在哪」。
- **§4～§7 是正文**：数据结构、前向传播、反向传播、链式法则，共用同一个手算例子。
...
`e.backward()` 之后必须是 `e.data == 30`、`a.grad == 21`、`b.grad == 16`（已在本机跑过验证）。对不上就说明图、反向或链式法则哪一环断了。前向只填 `data`；梯度是反向写进去的。
...
| 层 | 字段 | 前向用 | 反向用 | 画图用 |
|----|------|--------|--------|--------|
| 数值 | `data` | 读、写出新节点 | 乘法等局部导数要读它 | 左格 |
| 数值 | `grad` | 不读 | 写 `d(根)/d(自己)` | 右格 |
| 图骨架 | `_prev` | 写入，之后不读 | 拓扑 DFS 的邻接表 | 边 |
| 图语义 | `_backward` | 写入一个闭包 | 真正执行链式法则 | 不用 |
| 标签 | `_op` | 写入 | 不用 | 算子盒子 |

叶子（用户 `Value(2.0)`，以及 `Neuron` 的 `w`/`b`）：`_prev` 空，`_op` 空，`_backward` 是空 `lambda`。中间节点：`_prev` 指向输入，`_backward` 指向「怎么把 `out.grad` 分给输入」。

没有 `requires_grad`：每个节点都有 `grad`。没有 `generation` / `creator`（那是本仓库 `microai.Variable` 的拆法）。
